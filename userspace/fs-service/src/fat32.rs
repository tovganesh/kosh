//! FAT32, read-only, in ring 3.
//!
//! ## Where this came from
//!
//! `kernel/src/fs/fat32.rs`, almost line for line. That file is correct and
//! tested against a host-mounted image, so the port deliberately changes as
//! little as possible: the BPB parsing, the geometry validation, the cluster
//! chain walk, the long-filename reassembly, the path lookup and `read_at` are
//! the kernel's code. The only edits are the three sector reads, which now go
//! through [`crate::block`] — an IPC round trip to `userspace/ata-driver`
//! instead of a call into the kernel's block layer — and `read_cluster`, which
//! no longer gets its whole cluster in one transfer because a message holds four
//! sectors.
//!
//! Nothing else needed to change, and that is the interesting result. A
//! filesystem is arithmetic on sector contents; it does not care what privilege
//! level performs the arithmetic, and it did not need one line of kernel service
//! beyond "give me sector N".
//!
//! ## Why FAT32 and not ext4
//!
//! The `ext4.rs` and `vfs.rs` that used to sit next to this file are instructive
//! about why it exists: `read_block` filled the buffer with zeroes, `write_block`
//! returned success without writing, the superblock was fabricated in code,
//! `mtime` was the literal 1234567890, and `read_dir` returned only `.` and
//! `..`. They were never connected to a disk, because there was no disk driver.
//! They are deleted now.
//!
//! FAT32 is a few hundred lines to read correctly, and — more usefully — the
//! image can be mounted on the development host, so every answer this server
//! gives can be checked against what is actually on the disk.
//!
//! ## What is here
//!
...
//! allocating clusters and updating both FAT copies is a separate problem, and a
//! read-only filesystem that is correct beats a read-write one that is not. The
//! block protocol has no write operation either, so there is nothing to build on
...
pub enum FsError {
    ...
    CorruptChain,
    /// Carried over from the kernel's enum so the error mapping in `main.rs`
    /// stays a copy of `fs_error_to_syscall`. Nothing in this file produces it
    /// — path length is bounded by the 4096-byte message cap instead.
    #[allow(dead_code)]
    NameTooLong,
}
...
impl DirEntry {
    /// Unused by the server — the protocol has no write path to refuse — but
    /// kept so this stays a faithful copy of the kernel's type.
    #[allow(dead_code)]
...
impl Fat32 {
    ...
    pub fn mount(lba: u64) -> Result<Self, FsError> {
        ...
        // hostile BPB otherwise turns into out-of-range reads — and in this
        // process those reads are requests to another process, which will refuse
        // them with STATUS_OUT_OF_RANGE rather than fault. That is a nicer
        // failure than the kernel's was, but it is not a reason to skip the
        // checks: the arithmetic below can still overflow into a *valid* sector.
    ...
    ///
    /// This is the hottest read in the whole server — one per link of every
    /// chain — and it is why `block::read_block` caches a sector. Consecutive
    /// clusters share a FAT sector 127 times out of 128, so with the cache this
    /// costs no IPC at all in the common case.
    ...
    ///
    /// `block::read_blocks` splits this into runs of four sectors, so a 4 KiB
    /// cluster is two messages rather than one transfer. That is the honest cost
    /// of the message cap; it is not worth working around, because the
    /// alternative is a shared-memory grant and this program does not need one
    /// badly enough to justify the mechanism.
    ...
    /// entry of its own on disk. Its name is `/`, which is what a client's
    /// `stat("/")` sees — the kernel's `sys_stat` reported the same thing, so
    ...
    /// file in N chunks into O(N^2) disk traffic, and the protocol above this
    /// makes N large: a client gets at most 4000 bytes per message, so even a
    /// modest file is read in dozens of calls.
    pub fn read_at(
        &self,
        entry: &DirEntry,
        offset: u32,
    ...
    ///
    /// The server never calls this — every client read goes through `read_at`,
    /// because a reply holds 4000 bytes and buffering a whole file in a 256 KiB
    /// heap to answer one of them would be a way to run out of memory. It is
    /// kept because it is the kernel's function and the port is meant to be
...
///
/// The case is whatever is on disk, which for anything mtools or Linux wrote is
/// upper. `.` and `..` come out of here unchanged and are returned to clients as
/// ordinary entries, exactly as the kernel's `getdents` did — they are real
/// directory entries in a FAT subdirectory, and hiding them would be this
/// server inventing a policy.
